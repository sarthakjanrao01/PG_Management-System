...
import { deleteBookings, fetchBookings } from "../../../Shared/Store/BookingAuthStore";
  ...
  useEffect(() => {
    const getBookings = async () => {
      try {
        ...
        const filteredBookings = data.filter((booking: BookingModel) => booking.status === "Pending");
  ...
  const handleDelete = async (bookingId: string) => {
    if (window.confirm("Are you sure you want to delete this Booking?")) {
      ...
      try {
        ...
        alert("Booking deleted successfully!"); // Notify user of success
      } catch (error) {
  ...
  return (
    <div className="bg-white">
      ...
      <div className="flex justify-between items-center border-b pb-4 mb-4">
        <h2 className="text-2xl font-bold text-blue-500">Pending Booking</h2>
      </div>
