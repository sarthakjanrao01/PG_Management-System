...
const CanceledBooking: React.FC = () => {
    ...
    const getBookings = async () => {
      ...
      try {
        ...
        const filteredBookings = data.filter(
          (booking: BookingModel) => booking.status === "Canceled"
        );
  ...
  const handleDelete = async (bookingId: string) => {
    if (window.confirm("Are you sure you want to delete this Booking?")) {
      setDeleting(true); // Start deleting loading state
      try {
        await deleteBookings(bookingId); // Call API to delete the booking

        // Update the bookings state to filter out the deleted booking
        ...
        setDeleting(false); // End deleting loading state
      }
    }
  };
  ...
  return (
    <div className="bg-white">
      ...
      <div className="flex justify-between items-center border-b pb-4 mb-4">
        <h2 className="text-2xl font-bold text-blue-500">Canceled Booking</h2>
      </div>
